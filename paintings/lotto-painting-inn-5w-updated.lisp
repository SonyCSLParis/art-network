...
(defmethod get-node-color ((type (eql :cv)))
  (declare (ignore type))
  "#4FF7CA")
...
(defmethod get-node-color ((type (eql :kb-zeri))) ;zeri
  (declare (ignore type))
  "#FFA164")

(defmethod get-node-color ((type (eql :kb-arco))) ;arco
  (declare (ignore type))
  "#92694F")
...
(setq *secondary-where-question-nodes*
  (list (make-narrative-question :label "?Creation place")
        (make-narrative-question :label "?Display place")))

(setq *secondary-why-question-nodes*
  (list (make-narrative-question :label "?Function")
        (make-narrative-question :label "?Meaning or message")))
...
(inn-add-edges-to-node "?Where" *secondary-where-question-nodes*)
(inn-add-edges-to-node "?Why" *secondary-why-question-nodes*)
)


;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;Artwork ID on Knowledge bases ;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
;; Who is the author?
;; -------------------------
;;wikidata query: sparql-who-wd-author
(inn-add-node *lotto-inn*  (make-inn-node :label "Lorenzo Lotto" :type :kb-wikidata))
(inn-answer-question-by-adding-edge "?Author" "Lorenzo Lotto")
(inn-answer-question (inn-find-node-by-label (get-current-inn) "?Who"))
;; artist's name: 
;; (sparql-who-zeri-artist "47929")
;; answer: id: https://w3id.org/zericatalog/person/6225/lotto-lorenzo , label: "Lotto Lorenzo" 

(inn-add-node *lotto-inn*  (make-inn-node :label "Lotto Lorenzo" :type :kb-zeri))
(inn-answer-question-by-adding-edge "?Author" "Lotto Lorenzo")


;; when did he live?
;; -------------------------
;; wikidata query: sparql-who-wd-dates-person 
...
(inn-answer-question (inn-find-node-by-label (get-current-inn) "?When"))
...
(inn-add-node *lotto-inn* (make-inn-node :label "Venice" :type :kb-wikidata))
(inn-add-node *lotto-inn* (make-inn-node :label "Loreto" :type :kb-wikidata))
...
;;wikidata query: sparql-who-wd-author-other-artworks 
;;
...
(inn-answer-question (inn-find-node-by-label (get-current-inn) "?Other artworks"))
...
;; religious oder
;; wikidata query: sparql-who-wd-author-religious-order
(progn
...
;; type?
;; Wikidata P31
;; query: sparql-what-wd-type
(inn-answer-question-by-adding-edge-and-nodes "?Type" '("painting") :kb-wikidata)
...
;; title?
;; wdt:P1476
(inn-answer-question-by-adding-edge-and-nodes "?Title" '("Venere e Cupido") :kb-wikidata)
...
;; we add a relation between author and art movement because we discovered it through the author
(inn-add-edges-by-nodes-label "?Author" "?Art Movement")
(inn-answer-question-by-adding-edge-and-nodes "?Art Movement" '("scuola italiana" "scuola veneta") :kb-zeri)
...
(inn-answer-question (inn-find-node-by-label (get-current-inn) "?Subject"))
;; Zeri                                       
...
;; query: sparql-what-icon-level1
  ...
  (inn-add-label-edge-from-nodes-label "?Depicts" "rod" "first level")
  (inn-node-id (inn-find-node-by-label (get-current-inn) "?Depicts"))
...
;; Computer vision
(inn-add-node *lotto-inn* (make-inn-image   :type :inn-image
                                            :label "Computer Vision analysis"
                                            :description "Computer Vision analysis"
                                            :url "https://inn-painting-art-interpretation.streamlit.app/~/+/media/c50272826fa227d87996f0c6755b09986b33cae2797830346fba7343.png"))
(inn-add-label-edge-from-nodes-label "?Depicts" "Computer Vision analysis" "Computer Vision results")
(inn-add-edges-to-node "Computer Vision analysis" (list (make-inn-node :label "person" :type :cv)
                                                        (make-inn-node :label "person" :type :cv)
                                                        (make-inn-node :label "hat" :type :cv)))
...
;; wikidata query: sparql-what-wd-painting-main-subject-same-movement 
...
; zeri
(inn-add-label-edge-and-node "?When" "1520-1556" "date of creation" :kb-zeri)
...
;; no information about it. The Question is partially anwered with the authror's locations. 

(inn-add-edges-by-nodes-label "?Location" "?Creation place") 
...
;; Lotto Lorenzo and Lorenzo Lotto
;; reconciliation through the common ULAN
(inn-add-label-edge-from-nodes-label "Lorenzo Lotto" "Lotto Lorenzo" "same as")
